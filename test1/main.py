...
class CustomCrew:
    ...
    def run(self):
        ...
        implementation_task = tasks.implementation_task(programmer_agent, programmer_tools, [self.user_input, architecture_task, extracting_task])
        reviewing_task = tasks.reviewing_task(reviewer_agent, reviewer_tools, [architecture_task, implementation_task, extracting_task])

        crew = Crew(
            agents=[architect_agent, programmer_agent, extractor_agent, reviewer_agent],
            tasks=[architecture_task, implementation_task, extracting_task, reviewing_task],
            verbose=True,
        )
